...
void* worker(void* p) {
	while (contador_conj_jobs < LIMITE_CONJ_JOBS) {
		...
		pthread_barrier_wait(&barreira);//Usamos a barreira nas threads para sincronizar com a barreira da main
...
int main(void) {
	...
	pthread_barrier_init(&barreira, NULL, QTD_THREADS + 1);//+1, para incluir a thread principal
	...
	while (contador_conj_jobs < LIMITE_CONJ_JOBS) {
		...
		pthread_barrier_wait(&barreira);//Usamos a barreira aqui pra sincronizar
